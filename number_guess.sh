...
if [[ -z $USERNAME_RESULT ]]
  then
    ...
    BEST_GAME=$($PSQL "SELECT MIN(number_guesses) FROM games LEFT JOIN users USING(user_id) WHERE username='$USERNAME'")
...
INSERT_GAME_RESULT=$($PSQL "INSERT INTO games(user_id, secret_number, number_guesses) VALUES ($USER_ID_RESULT, $SECRET_NUMBER, $GUESS_COUNT)")
